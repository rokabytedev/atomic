...
export default function TrackHabitPage() {
        ...
        <button
          ...
          className={`text-sm py-2 rounded-full transition-colors ${
            !isCurrentMonth 
              ...
              : isToday
                ? 'bg-white text-[#047857] font-bold shadow-md ring-2 ring-[#047857]'
                : isCheckedIn
                  ? 'bg-[#e6f7f2] text-[#047857] font-semibold hover:bg-[#d1f2e6]'
  ...
  if (!habit) {
    return (
      <div className="bg-[#f9fafb] text-[#111827] min-h-screen flex items-center justify-center">
        <p>Loading habit...</p>
      </div>
    );
  }
        ...
        <header className="w-full">
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between py-5">
              <Link className="flex items-center gap-2 text-[#6b7280] hover:text-[#111827] transition-colors" href="/">
        ...
        <main className="flex-grow w-full max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="space-y-10">
            <div className="text-center">
              <h2 className="text-4xl font-bold tracking-tight text-[#111827] sm:text-5xl">{habit.habitAction}</h2>
            </div>
            <div className="bg-white rounded-xl shadow-sm p-6 border border-[#e5e7eb]">
              ...
              <div className="space-y-2 text-[#6b7280]">
                <p><span className="font-medium text-[#111827]">Identity:</span> {habit.identity}</p>
                <p><span className="font-medium text-[#111827]">Cue:</span> {getCueText()}</p>
                {habit.temptationBundle && (
                  <p><span className="font-medium text-[#111827]">Reward:</span> {habit.temptationBundle}</p>
                )}
              </div>
            </div>
            <div className="bg-white rounded-xl shadow-sm p-6 border border-[#e5e7eb]">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div className="bg-white rounded-xl shadow-sm p-6 border border-[#e5e7eb] text-center">
                ...
                <p className="text-4xl font-bold text-[#047857] mt-2">{calculateStreak()} days</p>
              </div>
              <div className="bg-white rounded-xl shadow-sm p-6 border border-[#e5e7eb] text-center">
                <p className="text-sm font-medium text-[#6b7280]">Total Completions</p>
                <p className="text-4xl font-bold text-[#047857] mt-2">{habit.checkins.length}</p>
              </div>
            </div>
          </div>
        </main>
